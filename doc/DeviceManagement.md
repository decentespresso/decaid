...
Decent Scale sleep is capability-gated. After a connection is confirmed the scale runs an unawaited profile negotiation: the canonical LED ON/status command (`0A 01`, heartbeat byte `00`) followed by the HDS `0x22` voltage probe. Only a valid `0x22` response promotes the connection to Half Decent Scale capabilities (extended commands and power off). SoftSleep (`0A 04`) is granted only when that connection also reports a modern firmware version (major `>= 3`); HDS that reports no version (pre-3.0.1) and unknown/original scales disconnect for sleep (`disconnectsToSleep`) and re-negotiate on wake. A failed SoftSleep write disconnects even while the native GATT state still reports connected. Power-off is withheld unless the profile proves support. Capabilities are re-confirmed on every physical connection and are discarded if a response arrives after sleep was requested.
