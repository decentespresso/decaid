...
A shorter frame must not be accepted: otherwise the last payload byte can be
mistaken for the checksum and a zero-weight snapshot can satisfy readiness.
...
The Eclair connect timeout records how many notifications arrived and the last
frame that failed validation. This distinguishes a dead subscription from a
frame format the parser rejects.

The command characteristic advertises write-with-response only on current
firmware. `AtomheartScale` therefore issues timer and tare commands with a
response. `UniversalBleTransport.write` retries a write-without-response once
with response only when the platform rejects the requested property; a rejected
write-with-response is surfaced as-is and never downgraded. Darwin, Android,
and Windows validate the property before radio dispatch, so the retry cannot
duplicate a command; BlueZ does not report these property errors.

`_handleGattError` logs an unmapped `UniversalBleException` before rethrowing.
REST handlers that turn an exception into a 500 body log it too.
